...
type NestedHeaderSettings = Exclude<ArrayElement<ArrayElement<Handsontable.GridSettings["nestedHeaders"]>>, string>[][];
type MergeCellSettings = Exclude<Handsontable.GridSettings["mergeCells"], boolean>
      ...
      case "data": {
        ...
        for (const [col, d] of rowData.entries()) {
          ...
          if (results.groups.height || results.groups.width) {
            ...
            (HOTConfig.mergeCells as MergeCellSettings).push({
              col,
              row,
              colspan: cellWidth,
              rowspan: cellHeight
...
export const hotLanguageSerialize = (htable: Handsontable): string => {
    ...
    const headingData: NestedHeaderSettings = htable.getSettings().nestedHeaders as any;
    const mergedCells: MergeCellSettings = htable.getSettings().mergeCells as any;
